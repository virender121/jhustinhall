...
app.post('/api/paypal/create-order',async(req,res,next)=>{
    ...
    try{
        ...
        if(items.some(i=>i.currency!==currency))return res.status(400).json({error:'Mixed currencies are not supported'});
        const totalCents=items.reduce((s,i)=>s+i.unit_price_cents*i.quantity,0);
        if(totalCents<=0)return res.status(400).json({error:'Invalid order total'});
        const orderNumber=`JH-${Date.now().toString(36).toUpperCase()}-${crypto.randomBytes(6).toString('hex').toUpperCase()}`;
localOrder=await tx(async c=>{
    const {rows:[o]}=await c.query(`INSERT INTO orders(
        order_number,
        status,currency,
        total_cents,
        customer,
        shipping,
        notes) VALUES($1,'PENDING',$2,$3,$4,$5,$6) RETURNING id,order_number`,[
    orderNumber,
    currency,
    totalCents,
    customer,
    shipping,notes]);
    for(const i of items)await c.query('INSERT INTO order_items(order_id,product_id,name,unit_price_cents,quantity) VALUES($1,$2,$3,$4,$5)',[o.id,i.product_id,i.name,i.unit_price_cents,i.quantity]);return o;});
